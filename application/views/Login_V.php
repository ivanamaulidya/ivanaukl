        ...
        <div class="nk-block toggled" id="l-login">
            <form id="login" method="POST">
                <div class="nk-form">
                    ...
                    <button type="submit" class="btn btn-login btn-success btn-float"><i class="notika-icon notika-right-arrow right-arrow-ant"></i></a>
                </div>
            </form><br>
                <a class="btn btn-info mt-30" data-toggle="modal" data-target="#ADD"></i>DAFTAR</a>
        </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="daftar" action="#" method="post">
                        ...
                        <input type="text" name="alamat" class="form-control" placeholder="Alamat Pelanggan" required><br>
                        <label>No Telp</label><br>
                        <input type="text" name="telp" class="form-control" placeholder="Telp Pelanggan" required><br>
...
<script>
    $("#login").submit(function(event)
    {
        ...
        var data_masuk = $('#login').serialize();
        $("pesan").html("Loading....");
        $.ajax({
            url : "<?=base_url()?>index.php/Login/masuk",
            type : "post",
            data : data_masuk,
            dataType : "json",
            success:function(hasil)
            {
                if(hasil['status']==1)
                {
                    $("#pesan").html("Sukses masuk");
                    setTimeout(function(){
                        location.href="<?=base_url()?>index.php/Pelanggan";  
                    }, 2000);
                }
                else if(hasil['status']==2)
                {
                    $("#pesan").html("Sukses masuk sebagai Admin");
                    setTimeout(function(){
                        location.href="<?=base_url()?>index.php/Masakan";  
                    }, 2000);
                }
                else if(hasil['status']==3)
                {
                    $("#pesan").html("Sukses masuk sebagai Kasir");
                    setTimeout(function(){
                        location.href="<?=base_url()?>index.php/Kasir";  
                    }, 2000);
                }
                else
                {
                    $("#pesan").html("Username dan Password salah");
                }   
            }
        });
    });
    ...
    {
        event.preventDefault();
        var daftar = $("#daftar").serialize();
        $("#keterangan").html("<p class='btn btn-danger'>Sedang Memeriksa......</b></p>");
        $.ajax({
            url : "<?=base_url()?>index.php/Login/Tambah",
            type : "post",
            data : daftar,
            dataType : "json",
            success:function(hasil)
            {
                if(hasil['status'] == 1)
                {
                    $("#keterangan").html(hasil['keterangan']);
                    $("[name=nama]").val('');
                    $("[name=alamat]").val('');
                    $("[name=telp]").val('');
                    $("[name=username]").val('');
